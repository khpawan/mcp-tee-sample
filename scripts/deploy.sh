...
echo ""
echo "Next steps:"
echo "  1. Populate secrets in Key Vault:"
echo "     az keyvault secret set --vault-name <kv-name> --name github-token --value <token>"
echo "     az keyvault secret set --vault-name <kv-name> --name db-connection-string --value <connstr>"
echo "     az keyvault secret set --vault-name <kv-name> --name webhook-url --value <url>"
echo ""
echo "  2. Update key-release-policy.json with the policy hash above"
echo ""
echo "  3. Verify attestation:"
